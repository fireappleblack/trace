...
| `trace-admin/app_admin.py`, `admin.html`, `Containerfile`, `requirements.txt`, `README.md` | Zip-game dev | **Separate** admin service + single-file admin client + its image/docs. Imports the shared `trace-server/db.py`; publishes to GHCR (`ghcr.io/fireappleblack/trace-admin`). The public app/client carry no admin code |
| `trace-admin/deploy/admin-k8s.yaml`, `apply-admin.sh`, `.secrets.env.example` | Zip-game dev | Admin Deployment/Service/Ingress + secret tooling. **Caveat:** the Ingress targets `admin.zip.hsabren.co.uk` — its DNS, TLS cert, and **edge gate** (Cloudflare Access / Traefik middleware) are **Infrastructure** |
| `trace-core/trace-core.js` | Zip-game dev | Shared game-logic core (generator/solver/signature/measures), extracted verbatim from `trace.html`. Loaded by `trace.html` + `admin.html`, importable by Node. Regenerated by `extract_core.js`; equivalence proven by `harness_golden.js core` |
| `trace-core/harness_golden.js`, `extract_core.js`, `golden.json` | Zip-game dev | Dev only (not shipped): the golden/equivalence net, the core generator, and the captured baseline. `golden.json` is a generated blob (committed, not flattened) |
...
├── trace-admin/                    # ADMIN (zip-game dev) — separate from the app
│   ├── app_admin.py admin.html Containerfile requirements.txt README.md
│   └── deploy/
│       └── admin-k8s.yaml apply-admin.sh .secrets.env.example
│                                   # ↑ DNS/cert/edge-gate for admin.zip.… = INFRA
├── trace-core/                     # SHARED CORE (zip-game dev)
│   ├── trace-core.js               # shipped: loaded by trace.html + admin.html
│   └── harness_golden.js extract_core.js golden.json   # dev only (not shipped)
