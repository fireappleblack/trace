...
```bash
# assuming a Postgres reachable at localhost:5432 with a 'trace' database
...
**both** `POSTGRES_PASSWORD` and the password embedded in `DATABASE_URL` (they
must match). For anything real, prefer creating the Secret out-of-band rather
than committing it.
