...
- **`trace.html`** — the single, canonical app. Drop it on any web server
  (or open via `file://` — most features work, except those that need a
  server, like leaderboards). Generator, solver, hints, local
  SQLite-in-browser persistence, the lot.

- **`trace-server/`** — Flask app that serves `../trace.html` (the very same
  file — no duplicate copy) plus a small REST API for cross-user
  leaderboards, aggregates, and insights. SQLite by default; switch to
  Postgres via `DATABASE_URL`. Includes a `Containerfile` and k3s manifests
  for container deployment. See its own README for setup.

There is exactly one `trace.html`. The server resolves it one directory up,
so the standalone copy and the served copy can never drift apart.

## Running it

- **Standalone:** open `trace.html` in a browser.
- **With the server (local):** `cd trace-server && pip install -r requirements.txt && python app.py`, then open <http://localhost:5000>.
- **As a container:** build from this directory with
  `podman build -f trace-server/Containerfile -t localhost/trace:latest .`
  then run it (SQLite for a quick smoke test, or point `DATABASE_URL` at a
  Postgres).
- **On k3s with a dedicated Postgres:** see `trace-server/README.md` →
  "Deploy to k3s". In short: side-load the image into containerd, then
  `kubectl apply -f trace-server/deploy/postgres.yaml` (the dedicated
  database) followed by `kubectl apply -f trace-server/deploy/trace-k8s.yaml`
  (the app). The app is stateless against Postgres and runs multiple replicas;
  it waits for the database to be ready and recovers automatically if the
  database restarts.
